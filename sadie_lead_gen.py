...
async def sniff_network_for_engine(network_hosts: set, hotel_domain: str):
    """Check captured network requests for booking engine domains."""
    for host in network_hosts:
        engine_name, pat = detect_engine_from_domain(host)
        if engine_name:
            return (engine_name, host, "network_sniff")
    return ("", "", "")
    ...
    async with semaphore:
        ...
        try:
            ...
            if engine_name in ("", "unknown", "unknown_third_party", "proprietary_or_same_domain"):
                net_engine, net_domain, net_method = await sniff_network_for_engine(network_hosts, hotel_domain)
                if net_engine:
        ...
        with open(input_csv, newline="", encoding="utf-8") as f:
            ...
            for row in reader:
                hotels_to_process.append({
                    ...
                    "phone": row.get("phone", row.get("formatted_phone_number", "")),
                    "address": row.get("address", row.get("formatted_address", "")),
                    "rating": row.get("rating", ""),
                    "review_count": row.get("review_count", row.get("user_ratings_total", "")),
        ...
        with open(output_csv, mode, newline="", encoding="utf-8") as f:
            ...
            for coro in asyncio.as_completed(tasks):
                ...
                if since_flush >= flush_every:
                    f.flush()
                    since_flush = 0
                    log(f"[Progress] {stats['processed']}/{len(hotels_remaining)} processed, {stats['known_engine']} known engines, {stats['errors']} errors")
...
def main():
    ...
    parser.add_argument("--max-results", type=int, default=0,
                        help="Max hotels to scrape (0 = no limit)")
    ...
    parser.add_argument("--concurrency", type=int, default=5,
                        help="Parallel browser tabs (default: 5)")
    parser.add_argument("--headed", action="store_true",
                        help="Show browser UI")
    parser.add_argument("--pause", type=float, default=0.5,
                        help="Pause between hotels (default: 0.5s)")
    
    # Skip scrape option (use existing CSV)
    parser.add_argument("--skip-scrape", action="store_true",
                        help="Skip Google Places scrape, use --input CSV instead")
    parser.add_argument("--input", default="",
                        help="Input CSV file (used with --skip-scrape)")
    
    args = parser.parse_args()
    
    api_key = os.getenv("GOOGLE_PLACES_API_KEY")
    if not api_key and not args.skip_scrape:
        raise SystemExit("Missing GOOGLE_PLACES_API_KEY in .env (or use --skip-scrape with --input)")

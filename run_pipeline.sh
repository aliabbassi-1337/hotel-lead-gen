...
for SCRAPER_FILE in "${FILES[@]}"; do
    ...
    if [ "$SKIP_ROOM_COUNT" = false ]; then
        ...
        python3 scripts/enrichers/room_count_groq.py --input "$LEADS_FILE" --concurrency 3 || warn "Groq room count failed"

        header "ROOM COUNT ENRICHMENT (Google AI fallback)"
        python3 scripts/enrichers/room_count_google.py --input "$LEADS_FILE" --concurrency 5 || warn "Google AI room count failed"
    fi
